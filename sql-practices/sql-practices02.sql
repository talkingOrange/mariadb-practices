...
SELECT 
    MAX(salary) AS '최고연봉', MIN(salary) AS '최저연봉'
FROM
    salaries
WHERE
    to_date = '9999-01-01';
...
select period_diff(date_format(min(birth_date), '%y'), date_format(curdate(), '%y'))  as '어린사원',
period_diff(date_format(max(birth_date), '%y'), date_format(curdate(), '%y')) as '연장자'
 from employees
 ;
